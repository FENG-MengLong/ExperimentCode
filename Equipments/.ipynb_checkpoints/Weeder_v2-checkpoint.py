...
class Weeder:
	...
	def move(self, header: str, position: int, progress = True):
		...
		if position > step_max:
			...
			step_change = int(current_position) - position
			...
			print(f"Old position: {current_position}\nNew position: {new_position}" )
	...
	def advance(self, header: str, num_step: int):
		...
		if new_position > step_max:
			...
			for i in range(abs(step_change)):
				...
				if progress:
					print(f"start at {current_position} --> {(i+1) * abs(num_step)/step_change + current_position} --> {position}",end= "\r",flush= True)
