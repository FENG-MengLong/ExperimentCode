...
class MCBOX():
    ...
    def configure(self, board_num: int, channels: int, bipolar = False):
        try:
            if bipolar:
                for channel in channels:
                    ul.set_config(InfoType.BOARDINFO, board_num, channel, BoardInfo.DACRANGE, ULRange.BIP10VOLTS)
                    print(f'Channel {channel} has been set to the bipolar voltage range.') # optional
            else:
                for channel in channels:
                    ul.set_config(InfoType.BOARDINFO, board_num, channel, BoardInfo.DACRANGE, ULRange.UNI10VOLTS)
    ...
    def set_voltage_Nchan(self, board_num: int ,channels: int, voltages: float, bipolar = False):
        try:
