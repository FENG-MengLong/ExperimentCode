...
class PlotSaver:
    ...
    def Plot3D(
        ...
        _save_csv(
            ...
            data_hash, plot_id, csv_filename, today_display, notes
        )
    ...
    def Plot2D(
        ...
        # --- plot, even if duplicate ---
        _showPlot2D(
            x, y, Z, xlabel, ylabel, zlabel, title, figsize, cmap,
            y_index, y_value, notes
        )
    ...
    def save_frame(
        ...
        """
        Save a 2D image frame using the same date-folder / ID / hash policy
        as Plot3D/Plot2D.
...
def validate_input(x, y, Z):
    ...
    if y is not None and Z.shape != (len(x), len(y)):
        raise ValueError(
            f"Z shape {Z.shape} must be (len(x), len(y)) = ({len(x)}, {len(y)})"
        )
...
def _showPlot3D(x, y, Z, xlabel, ylabel, zlabel, title, figsize, cmap, notes):
    ...
    ax = fig.add_axes([
        0.12,
        extra_bottom / (height + extra_bottom),
        0.78,
        height / (height + extra_bottom)
    ])

    ax_notes = fig.add_axes([
        0.05,
        0.01,
        0.90,
        (extra_bottom - 1) / (height + extra_bottom)
    ])
    ax_notes.axis("off")
    ...
    if y_index is not None:
        if isinstance(y_index, (list, tuple)) and len(y_index) == 2:
            ...
            for i, idx in enumerate(range(start, end)):
                ax.plot(x, Z[:, idx], color=colors[i], label=f"{ylabel}={y[idx]:.3g}")
        else:
            # single index
            idx = int(y_index)
